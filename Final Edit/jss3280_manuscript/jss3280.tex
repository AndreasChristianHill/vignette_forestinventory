...
\Address{
  ...
  E-mail: \email{andreas.hill@forestinventory.de}\\
...
% \input{JStatSoft_forestinventory_master-concordance}
...
% While commercial enterprises have already been offering multiphase inventory analyses, freely available software for application of multiphase estimators has been rare. Two exceptions are the \proglang{R} package \pkg{JoSAE} by \citet{josae2015} and the package \pkg{maSAE} by \citet{cullmann2016}. However, a more comprehensive software package covering a larger variety of sample designs and estimators for forest inventories has not yet been available. The motivation behind the \proglang{R} package \pkg{forestinventory} is to offer scientists and forest practitioners a comprehensive set of multiphase estimation procedures that provide the advantages of a) full transparency of the implemented estimation methods, b) free access also for (small) forest enterprises who cannot afford commercial services, and c) implementation in an open source software that provides high flexibility to be incorporated in operational forest information systems.\par
% 
...
Two-phase sampling for stratification is applied if the model only contains categorical variables, in this case the factor variable \code{stage}. Linear regression models only fitted with categorical variables produce ANOVA models, which when used in multiphase regression estimators, is equivalent to post-stratification. For our example, this means that the model predictions are simply the means of the terrestrial response values within each development stage (within-strata means).
...
\begin{Sinput}
R> library("rmutil")
R> int2(target.suface, a = c(0,0), b=c(3,2))/6
R> int2(target.suface, a = c(0.5, 0.3), b=c(2, 1.3))/(1*1.5) 
\end{Sinput}
\end{Schunk}
...
\begin{Schunk}
\begin{Sinput}
R> sample.generator<- function(n1, n2, target.surface=target.surface){
