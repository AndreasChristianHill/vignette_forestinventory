...
\section{Twophase Estimators and their Application}
...
\begin{align}
  \hat{\var}(\hat{Y}_{reg2p,ex}) & :=\hat{\bar{\pmb{Z}}}^t\hat{\pmb{\Sigma}}_{\hat{\pmb{\beta}}_{s_2}}\hat{\bar{\pmb{Z}}} \label{eq:gw_var_2p_reg_ex}\\
...
\begin{align}
  \hat{\var}_{ext}(\hat{Y}_{reg2p,ex}) & = \frac{1}{n_2}\frac{1}{n_{2}-1}\sum_{x \in {s_2}}\Big(Y(x)-\bar{Y}\Big)^2 \label{eq:varexternal_2p_reg_ex} \\
  \hat{\var}_{ext}(\hat{Y}_{reg2p,nex}) & =
...
Note that the structure of the data frame has to meet certain requirements to be used by the \code{twophase} and \code{threephase} functions of the package. Firstly, a row in the data frame corresponds to a sample locations while the columns specify the attributes asociated to the respective sample location. Note that for global two-phase estimation, the data frame has to comprise a column that indicates whether a row corresponds to a $s_1$ or $s_2$ sample location (in \code{grisons}, this is the column \code{phase_id_2p}).\par

In order to apply the \textbf{non-exhaustive two-phase estimation with boundary weight adjustment} to the \code{grisons} data, we have to specify a) the regression model (\code{formula}) as specified in the \code{lm}-function \citep{R}, b) the data frame containing the inventory information (\code{data}), c) the name of the column specifying to which sample ($s_1$, $s_2$) an observation belongs (\code{phase.col}) as well as the indicator for the $s_2$, i.e. terrestrial sample (\code{terrgrid.id}), and d) the columname of the column containing the weights $w(x)$ of the boundary adjustments. The \textbf{non-exhaustive estimator with boundary weight adjustment} can thus be applied as follows:
\begin{small}
<<>>=
...
@
\end{small}
The \code{twophase}-function creates an \code{S3} object of class \code{"twophase"} and subclass \code{"global"}. A \textbf{summary of the estimation results} can be obtained by applying the \code{summary}-function to that object. It first provides a summary of the function call, the applied regression formula and the estimation method that was applied. The estimation results comprise the point estimate (\code{estimate}) as well as the design-based and external variance (\code{g_variance} and \code{ext_variance}). Further given are the sample sizes of the $s_1$ and $s_2$ sample (\code{n1}, \code{n2}) as well as the R$^2$ of the regression model (\code{r.squared}).
\begin{small}
<<>>=
summary(reg2p_nex)
@
\end{small}
If the exact means of the explanatory variables $\bar{\pmb{Z}}$ are available, the \textbf{exhaustive estimator} can be applied by additionally passing a vector containg the exact means to the optional argument \code{exhaustive}. The vector must be input in the same order that the \code{lm}-function processes a \code{formula} object including the intercept term whose exact mean will always be 1. We assume that in this case, the boundary adjustment has been applied when calculating the exact mean of $\pmb{Z}$.
...
@
\end{small}
Note that both variances of the exhaustive estimation are smaller than those of the non-exhaustive estimation. This is because we took out one component of uncertainty by substituting the estimated means of the explanatory variables $\hat{\bar{\pmb{Z}}}$ by their exact means $\bar{\pmb{Z}}$.
...
The package \pkg{forestinventory} offers 3 small area estimators. A general principle of all small area estimators of the package is to restrict the information to the small area $G$ whenever possible. An exception however is the retrieval of the regression coefficient vector $\hat{\pmb{\beta}}$ and its variance-covariance matrix $\hat{\pmb{\Sigma}}_{\hat{\pmb{\beta}}_{s_2}}$, which is always calculated using the entire sample $s_2$ according to equation \ref{normequ_simple} and \ref{eq:estvarmatrix}.\par

%----------------------------------------------- %
% \textbf{Small and Pseudo Small Area Estimator}\par

We first introduce the \textbf{small area estimator} (\textit{small}), which uses exhaustively computed explanatory variables, and its non-exhaustive version, the \textbf{\textit{pseudo}} \textbf{small area estimator} (\textit{psmall}). Their point estimates are given in equation \ref{eq:pointest_2p_reg_small} and \ref{eq:pointest_2p_reg_psmall}. As obvious, the globally derived regression coefficients are first applied to the exhaustively or non-exhaustively calculated means of the explanatory variables ($\bar{\pmb{Z}}_G$, $\hat{\bar{\pmb{Z}}}_G$) available at the sample location $s_{1,G}$ in the small area. A potential bias of the regression model predictions in the small area $G$ is then corrected by adding the mean of the residuals in $G$ to the first term. We call this the \textit{bias} or \textit{residual correction} term.
...
\begin{align}
  \hat{\var}(\hat{Y}_{G,small,2p}) & := \bar{\pmb{Z}}_G^t\hat{\pmb{\Sigma}}_{\hat{\pmb{\beta}}_{s_2}}\bar{\pmb{Z}}_G 
    + \frac{1}{n_{2,G}}\frac{1}{n_{2,G}-1}\sum_{x \in s_{2,G}} \Big(\hat{R}(x)-\hat{\bar{R}}_{2,G}\Big)^2  \label{eq:var_2p_reg_small} \\
  ...
  + \frac{1}{n_{2,G}}\frac{1}{n_{2,G}-1}\sum_{x \in s_{2,G}} \Big(\hat{R}(x)-\hat{\bar{R}}_{2,G}\Big)^2  \label{eq:var_2p_reg_psmall}
\end{align}
\end{subequations}

\begin{subequations}\label{eq:varext_2p_small_psmall}
\begin{align}
  \hat{\var}_{ext}(\hat{Y}_{G,small,2p}) & := \frac{1}{n_{2,G}}\frac{1}{n_{2,G}-1}\sum_{x \in s_{2,G}} \Big(\hat{R}(x)-\hat{\bar{R}}_{2,G}\Big)^2  \label{eq:varext_2p_reg_small} \\
  \hat{\var}_{ext}(\hat{Y}_{G,psmall,2p}) & := \frac{1}{n_{2,G}}\frac{1}{n_{2,G}-1}\sum_{x \in s_{2,G}} \Big(Y(x)-\bar{Y}_{2,G}\Big)^2 \nonumber \\ 
 &+ \Big(1-\frac{n_{2,G}}{n_{1,G}}\Big)\frac{1}{n_{2,G}}\frac{1}{n_{2,G}-1}\sum_{x \in s_{2,G}} \Big(\hat{R}(x)-\hat{\bar{R}}_{2,G}\Big)^2 \label{eq:varext_2p_reg_psmall}
\end{align}
\end{subequations}
...
If boundary adjustment is applied, the simple mean of the explanatory variable over the small area $\hat{\bar{\pmb{Z}}}_G=\frac{1}{n_{1,G}}\sum_{x \in s_{1,G}}\pmb{Z}(x)$ is replaced by its weighted version $\hat{\bar{\pmb{Z}}}_G=\frac{\sum_{x\in{s_{1,G}}}w(x)\bmp{Z}(x)}{\sum_{x\in{s_{1,G}}}w(x)}$, and likewise for exhaustively used auxiliary information.
...
In the \textit{small} and \textit{psmall} estimator, the crucial zero mean residual property for the small area $G$ is forced by the bias adjustment terms (equation \ref{eq:pointest_2p_reg_small}, \ref{eq:pointest_2p_reg_psmall}). However, the equations would considerably simplify if we could mathematically insure that the prediction model residuals over the entire area $F$ and the small area $G$ are by definition both zero at the same time, i.e. $\int_F R(x)dx=\int_G R(x)dx=0$. This is idea is realized in the \textbf{\textit{extended}} \textbf{synthetic} and \textbf{\textit{extended}} \textbf{pseudo synthetic estimator} (\textit{extsynth} and \textit{extpsynth}) by extending the vector of auxiliary information $\pmb{Z}}(x)$ by a binary categorical indicator variable which has the value \code{1} if the observation lies inside the small area $G$ and otherwise set to \code{0}. The \textit{extended} auxiliary vector thus reads as $\pmb{\mathbb{Z}}^t(x)=(\pmb{Z}^t(x),I_G(x))$ and corresponds to the classical ANOVA case. Note that this adjusts the intercept of the regression model for all data points of small area $G$ and thus insures that the mean of the residuals $\frac{1}{n_{2,G}}\sum_{x\in{s_{2,G}}}\mathbb{R}(x)$ in $G$ are always zero by construction.\par
...
\begin{equation}\label{eq:ext_estvarmatrix}
  ...
  \Big(\frac{1}{n_2^2}\sum_{x\in{s_2}}\hat{\mathbb{R}}^2(x)\pmb{\mathbb{Z}}(x)\pmb{Z}(x)^t\Big)
  \Big(\frac{1}{n_2}\sum_{x\in{s_2}}\pmb{\mathbb{Z}}(x)\pmb{\mathbb{Z}}^t(x) \Big)^{-1}
\end{equation}

The point estimates are defined as
...
and the design-based variances are
...
\hat{\bar{\pmb{\mathbb{Z}}}}_{1,G}^t\hat{\pmb{\Sigma}}_{\hat{\pmb{\theta}}_{s_2}}
\hat{\bar{\pmb{\mathbb{Z}}}}_{1,G}
...
(\pmb{\mathbb{Z}}(x)-\hat{\bar{\pmb{\mathbb{Z}}}}_{1,G})(\pmb{\mathbb{Z}}(x)-
\hat{\bar{\pmb{\mathbb{Z}}}}_{1,G})^t
\end{equation}
...
\begin{align}
  \hat{\var}_{ext}(\hat{Y}_{G,extsynth,2p}) & = \frac{1}{n_{2,G}} \frac{1}{n_{2,G}-1}\sum_{x\in{s_{2,G}}}(\mathbb{R}(x)-\bar{\mathbb{R}})^2 \label{eq:ext_varexternal_2p_extsynth} \\
  \hat{\var}_{ext}(\hat{Y}_{G,extpsynth,2p}) & =
 \frac{1}{n_{1,G}}\frac{1}{n_{2,G}-1}\sum_{x\in{s_{2,G}}}(Y(x)-\bar{Y}_{2,G})^2 \nonumber \\ 
 &+ (1-\frac{n_{2,G}}{n_{1,G}})\frac{1}{n_{2,G}}\frac{1}{n_{2,G}-1}\sum_{x\in{s_{2,G}}}(\mathbb{R}(x)-\bar{\mathbb{R}})^2 \label{eq:ext_varexternal_2p_extpsynth}
\end{align}
\end{subequations}
 
The estimators are called \textit{synthetic} since no residual term is present such as in the small and psmall estimator (equation \ref{eq:pointest_2p_reg_small} and \label{eq:pointest_2p_reg_psmall}) and thus look similar to the synthetic estimators (equation \ref{eq:pointest_2p_reg_synth}, \ref{eq:pointest_2p_reg_synth}, \ref{eq:var_2p_reg_synth}, \ref{eq:var_2p_reg_psynth}). Note that it can however be shown by a decomposition of $\hat{\theta}_{s2}$ that the residual correction is indeed included in the regression coefficient $\hat{\theta}_{s2}$ \citep{mandallaz2016}.
...
Small area estimations in the package \pkg{forestinventory} can be applied by additionally specifying the optional argument \code{small_area}. The input dataset has therefore to be extended by an additional column that specifies for each each row (i.e. observation of a sample location) the respective small area in which it is located. The argument \code{small_area} requires a \code{list}-object that comprises
...
The \textbf{extended pseudo small area estimator} (\textit{extpsynth}) can be applied by setting \code{unbiased=TRUE} and leaving the optional argument \code{psmall} to its default value of \code{FALSE}:
\begin{small}
<<>>=
...
In this case, the variance of the pseudo synthetic estimator is almost only half as high as under the \textit{psmall} and \textit{extended psynth} estimator. The reason for this is that the pseudo synthetic estimator neglects the uncertainty of the regression model predictions in terms of incorporating the residual variation (compare equation  \ref{eq:var_2p_reg_psynth} to \ref{eq:var_2p_reg_psmall} and XXX). Consequently, relying on the synthetic estimation would in this case lead to a severe overestimation of the estimation precision.\par

The \textbf{exhaustive versions of the small area estimators} (equations \ref{eq:pointest_2p_reg_small},\ref{eq:var_2p_reg_small},\ref{eq:varext_2p_reg_small},\ref{eq:pointest_2p_reg_synth},\ref{eq:var_2p_reg_synth}) can again be applied by specifying the optional argument \code{exhaustive}. Its application requires that we know the exact means of all explanatory variables within the small area of interest. In contrast to the \textit{global} estimators, the exact means have now to be delivered in the form of a \code{dataframe}, where each row corresponds to a small area, and each column specifies the exact mean of the respective explanatory variable. Note that likewise the case of global estimation, the order of the explanatory variables in the dataframe has to match the order in which they appear in the design matrix defined by the \code{lm} function in \proglang{R}. In order to tell \proglang{R} which row describes which small area, the row names have to match the respective names of the small areas specified in the \code{areas} argument.

For the \code{grisons} dataset, the exact means of the explanatory variables for the small areas used in \citet{mandallaz2013b} are thus defined by
\begin{small}
<<eval=FALSE>>=
colnames(lm(formula = tvol ~ mean + stddev + max + q75, data = grisons, x = TRUE)$x)
@
...
Comparing this result with the non-exhaustive counterpart \textit{extpsynth}, we see that the estimation uncertainty (defined by the variance) has again be significantly increased by substituting the estimated by the exact auxiliary means. In the function output, exhautive phases are indicated by a sample size of infinity (\code{Inf}). Also note that the function extracts the required exact means for small area \code{"A"} and \code{"B"} from the complete set of exact means defined in \code{true.means.Z.G}.
