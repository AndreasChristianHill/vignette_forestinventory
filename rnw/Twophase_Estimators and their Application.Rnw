...
Small area estimations in the package \pkg{forestinventory} can be applied by \textit{additionally} specifying the optional argument \code{small_area}. The input dataset has therefore to be extended by an additional column that specifies for each each row (i.e. observation of a sample location) the respective small area in which it is located. The argument \code{small_area} requires a \code{list}-object that comprises 1) the name of the column specifiying the small area of each observation (\code{sa.col}), 2) a vector specifying the small area(s) for which estimations are desired (\code{areas}), and 3) the argument \code{unbiased} that controls which of the three available estimators should be applied. In addition to global estimation, the \code{summary} now also displays the sample sizes in the respective small area (\code{n1G} and \code{n2G}).
...
Note that the point estimates and the design-based and external variances of both estimators are very similiar. However, the R$^2$s (\code{r.squared}) under the \textbf{extended pseudo small area estimator} are changing with the small area, while they are identical under the \textbf{pseudo small area estimator}. This is because under the \textbf{extended pseudo small area estimator}, the regression model is recalculated for each small area estimation after adding the indicator variable for the respective small area in the globally derived design matrices. In case of the \textbf{pseudo small area estimator}, the regression model stays the same for each small area estimation. Although the results of both estimators should always be close to each other, we recommend applying both estimators and compare the results afterwards in order to reveal unsuspected patterns in the data, particularly in the case of cluster sampling.\par
