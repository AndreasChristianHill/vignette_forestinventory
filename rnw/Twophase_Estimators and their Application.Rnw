...
The package again provides the \textit{design-based} (equation \ref{eq:var_2p_reg_small}, \ref{eq:var_2p_reg_psmall}) and \textit{external} variance (equation \ref{eq:varext_2p_reg_small}, \ref{eq:varext_2p_reg_psmall}) for the \textbf{small area estimator} and the \textbf{pseudo small area estimator}. Again note that all components are restricted those available at the sample locations $s_{1,G}$ and $s_{2,G}$ in the small area, with exception of the regression coeffient components $\hat{\pmb{\beta}}_{s_2}$ and $\hat{\pmb{\Sigma}}_{\hat{\pmb{\beta}}_{s_2}}$.
...
The \textbf{\textit{extended}} \textbf{synthetic} and \textbf{\textit{extended}} \textbf{pseudo synthetic estimator}

% - explain extended estimator
% - mention boundary weight adjustment likewise global estimation
...
In order to apply the \textbf{pseudo small area estimator} (\textit{psmall}), we set \code{unbiased=TRUE} as well as the optional argument \code{psmall=TRUE}:
\begin{small}
<<>>=
...
The small area functions all return an \code{S3} object of class \code{"smallarea" "twophase"}. In addition to global estimation, the \code{summary} output now also contains the sample sizes of each small area (\code{n1G} and \code{n2G}). Note that this information is also particularly provided in the \code{samplesizes} object returned by the function call.
\begin{small}
<<>>=
...
Comparing this result with the non-exhaustive counterpart \textit{extpsynth}, we see that the estimation uncertainty (defined by the variance) has again be significantly increased by substituting the estimated by the exact auxiliary means. In the function output, exhautive phases are indicated by a sample size of infinity \code{Inf}. Also note that the function extracts the required exact means for small area \code{"A"} and \code{"B"} from the complete set \code{true.means.Z.G}.
