...
\begin{align}
\hat{Y}_{G,extsynth}= \bar{\pmb{\mathbb{Z}}}^t_{G}\hat{\pmb{\theta}}_{s_2} \label{eq:pointest_2p_extsynth} \\
\hat{Y}_{G,extpsynth}=\hat{\bar{\pmb{\mathbb{Z}}}}_{G}^t\hat{\pmb{\theta}}_{s_2} \label{eq:pointest_2p_extsynth}
\end{align}
\end{subequations}
...
The estimators are called \textit{synthetic} since no residual term is present such as in the small and psmall estimator (equation ...) and thus look similar to the synthetic estimators (equation ...). Note that it can however be shown by a decomposition of $\hat{\theta}_{s2}$ that the residual correction is indeed included in the regression coefficient $\hat{\theta}_{s2}$ \citep{mandallaz2016}.
