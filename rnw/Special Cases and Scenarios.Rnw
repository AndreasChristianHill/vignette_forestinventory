...
Comparing the \textit{extpsynth} and \textit{psmall} estimates, we see that the point estimates are close and more important, the external as well as the design-based variances only differ marginally. This can be taken as a hint that the violation of the zero mean residual property can expected to be negligible in this case. We can also check the calculated mean of the residuals for the small area by
...
However, if an $s_2$ and/or $s_1$ point misses a variable which is only used in the full regression model (here \code{q75}), the function will \textbf{recode} the phase indicator of that point to $s_0$, since the point still provides the required information for the reduced model. If this concerns an $s_2$ sample loaction, the associated response variable can no longer be used.
