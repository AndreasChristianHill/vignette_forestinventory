...
where $\hat{Y}$ is the point estimate, $\var(\hat{Y})$ is the estimated variance, and $1-\alpha$ is the confidence level. Note that for three-phase sampling, $p$ constitutes the number of parameters used in the \textit{full} regression model. In case of cluster-sampling, $n_{2,G}$ is the number of terrestrial clusters (a cluster constitutes the sample unit and comprises multiple sample plots). In the package \pkg{forestinventory}, the confidence intervals for all estimation methods and estimators can be computed by the S3 generic function \code{confint()}, which requires an estimation object created by the \code{onephase()}, \code{twophase()} or \code{threephase()} function. For example, the 95\% confidence interval for the small area estimates by the \textit{extpsynth} estimator (section \ref{sec:twophase_sae}) are calculated by:
