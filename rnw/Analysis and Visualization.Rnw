...
% The \pkg{forestinventory} package also provides a \code{S3} generic plot method based on the \pkg{ggplot2} package \citep{ggplot2} to visualize the estimation results. Like \code{mphase.gain()}, also the plot method also requires an \code{esttable} object created by the \code{estTable()}-function. The \code{plot()}-function allows for two kinds of visualization: \textbf{1)} the \textit{\textbf{point estimates}} with overlayed confidence intervals, and \textbf{2)} the \textit{\textbf{estimation errors}}. Both plots can be obtained by passing the \code{esstable} object to the \code{S3} generic \code{plot} method. Whereas the estimation errors are plotted by default, the graphic of the point estimates and confidence intervals is returned when setting the argument \code{yvar = "estimate"}. Note that the graphics can arbitrarily be extended by additional \pkg{ggplot2} parameterizations.

The \pkg{forestinventory} package also provides a \code{S3} generic plot method based on the \pkg{ggplot2} package \citep{ggplot2} to visualize the estimation results in two ways: \textbf{1)} the \textit{\textbf{point estimates}} with overlayed confidence intervals, and \textbf{2)} the \textit{\textbf{estimation errors}}. Both plots can be obtained by passing the \code{esstable} object to the \code{plot()} function.
