...
\begin{small}
<<eval=FALSE>>=
# --- True spatial mean for Global and Small Area Surface Area: --- #
library(rmutil)
...
# --- function for global simulations --- #
run.global.sim <- function(num_replications, n1.vec){

  sfrac <- seq(0.02, 0.26, by=0.02) # sampling fractions
  true_value<- 39.17 # true population mean
...
library(forstinventory)
global.sims<- data.frame()
n1_sims<- c(250, 500, 750, 1000, 1500, 2000) # n1 sample sizes
