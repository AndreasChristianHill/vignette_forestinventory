...
Most forest inventories typically follow a procedure that involves randomly selecting points in the forest area defined by GPS coordinates to which inventorists are sent. At these points, the inventorist demarcates a sample plot, possibly with the help of an inclusion circle that indicates which trees above a certain threshold are to be selected for measurement. These measurements are then converted into local plot densities (e.g. the timber density in $\frac{m^3}{ha}$) before the desired estimator is applied.  The \pkg{forestinventory} package bridges this common inventory paradigm to the mathematics of the estimation process using what is called the \textit{infinite population approach}.  In this approach the sampling units are \textit{points} each of which has a local density that exists but might remain unobserved if that point was not selected in our inventory's sample. For the sake of argument, imagine that the local density for every point in the forested area was somehow known and we could visualize the spatial distribution of the local density function of a target variable (e.g. the timber volume) for the entire forest area (Figure \ref{fig:inf_pop_apr}). The \textit{population total} of the target variable (e.g. the total timber volume of the forest) is mathematically equivalent to the integral of that density function, which defines the volume under the green surface in (Figure \ref{fig:inf_pop_apr}). Clearly, evaluating the local densities for every one of the infinite point so that we can add them up is impossible. Thus, in the infinite population approach we are essentially approximating the integral of this unobserved density surface numerically by sampling directly from the surface with our terrestrial inventory sample. This is why the infinite population approach is also called the \textit{Monte-Carlo approach}.  

All estimators included in the \pkg{forestinventory} package basically rest upon these theoretical thoughts. The motivation for this decision was that no bothersome spatial correlations need to be considered as would be the case with geostatisical methods since we assume that the inventory points are selected uniformly and independently, i.e. the infinite analogy for simple random sampling. Furthermore, points can perfectly fill up any forest area with any amorphous shape, which is not possible if one were to use the \textit{finite populatoin approach} where the sampling plot defined around the point is considered the sampling unit.  However, this has the disadvatage that circles don't tesselate perfectly and polygons will inevitably distort and pixelate the forest boundary.  In practice, the population of sampling units in the finite approach will almost never perfectly cover the forests.  We also want to emphasize that the consideration of an underlying infinite population of sample points will also play an important role when incorporating auxiliary information in the frame of two- and three-phase estimation methods.
