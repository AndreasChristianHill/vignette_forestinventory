...
Most forest inventories gather the field information by randomly or systematically selecting points in the forest area defined by coordinates to which inventorists are sent. At these points, the inventorist measures sample trees located within a constructed circle (called sample plot) around the sample point $x$, and aggregates their individual characteristics (e.g. their individual timber volumes) to local plot densities (e.g. the timber volume density in $m^3/ha$). The estimators implemented in the \pkg{forestinventory} package use the so called \textit{infinite population approach} in order to bridge this sampling process to the mathematics behind the estimators. The infinite population approach assumes that the spatial distribution of the local density, $Y(x)$, over the forest area is determined by a mathematical continuous function, as visualized in Figure \ref{fig:inf_pop_apr}. The \textit{population total} of the target variable (e.g. the total timber volume of the forest) is mathematically equivalent to the integral of that density function, which defines the volume under the green surface in Figure \ref{fig:inf_pop_apr}. From this perspective, the challenging part of an inventory is that the form of this function, and thus $Y$, is unknown. Theoretically, we still could get $Y$ by observing the function value, i.e. the local density $Y(x)$ at each possible point $x$ over the function plain (i.e. the forest area) and sum those obervations up. However, we see at the first spot that this is impossible, because the number of function values $Y(x)$ and thus the number of possible sample points $x$ is $infinite$. So we are left with only a sample of $Y(x)$ from an infinite population of sample points in order to \textit{estimate} the integral $Y=\frac{1}{\lambda(F)}\int_{F}Y(x)dx$ by $\hat{Y}$. All estimators included in the \pkg{forestinventory} package basically rest upon this theoretical approach. A key point for practical implementation is that in the infinite population approach, a local density value $Y(x)$ is a point information associated to the sample point $x$, which constitutes the sample unit, but not to the sample plot area. This has some obvious practical advantages over the finite population approach, where the sample units actually are cirular or rectangular plots. One is the impossibility of a perfect tesselation of an amorphous forest area by any kind of plot-shape, and hence, the population in the finite approach is, strictly speaking, not defined. The consideration of an underlying infinite population of sample points will also play an important role when incorporating auxiliary information in the frame of two- and three-phase estimation methods.

\begin{figure}[htb]
\centering
\resizebox{0.5\hsize}{!}{\includegraphics{fig/OnePhase.png}}
\caption{Viisualization of the infinite population approach. The spatial distribution of a hypothetical density function for every point in a forested area is represented as wavy green surface.  Sample plots (grean circles) inform the inventorist of the value of the density function at that point.}
