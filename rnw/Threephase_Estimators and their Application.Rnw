...
Note that $\hat{R}(x)=Y(x)-\pmb{Z}^t(x)\hat{\beta}_{s_2}$ denotes the residuals of the full model, whereas $\hat{R}_{0}(x)=Y(x)-\pmb{Z}^{(0)t}\hat{\alpha}_{s_2}$ denotes the residuals of the reduced model. The \textbf{external variance} form under linear regression models is defined as
