...
The package allows for the calculation of \textbf{point estimates} under exhaustive and non-exhaustive use of the auxiliary information in the $s_0$ phase. Staying in the internal model approach again insures the zero mean residual property over $F$.
...
The \code{summary()} of a \code{threephase()}-function now recalls \textit{both} regression model formulas and also gives the coefficients of determination $R^2$ for both the reduced (\code{r.squared_reduced}) and the full (\code{r.squared_full}) regression model. We are told that including \code{stddev}, \code{max} and \code{q75} besides \code{mean} yields a 20 \% improvement in R$^2$. If we used \code{mean} as the only auxiliary variable at all sample locations (two-phase approach), we would see that the three-phase extention led to a considerable reduction in variance.
