...
\answerfin{The reviewer is concerned that more attention needs to be given concerning issues in boundary cases with the coverage rates.  Although the given explanation seems to have switched $\alpha$ and $1-\alpha$ in a few places the point is helpful and clear especially with the generously provided examples included for the finite population case.  We have reproduced similar results using simulation in the infinite population 2-phase context and have concluded that this indeed occurs with the sampling fraction between the first and second phases is less than $\alpha$.  For this reason we have expanded the confidence interval section to inform the reader of this situation.}
\\
...
\\
\answerfin{Reviewer 1 made a very valid criticism about designed-based confidence intervals when the sampling fraction is too small that we would like to include in the paper and Section 5 is the most logical place to put it.} \\
...
\answerfin{In this context "a biased estimate" is an estimate that is produced from a biased estimator.  It would be a bit tedious to explain every time that an given estimate was produced from a biased estimator.}
% --------------------------- %
...
\answerfin{It is elegant in the sense that the SAE estimator is unbiased merely by modifying the design matrix.  It is not necessary to refit every small area with an indicator, but it is  how the package implemented the estimator.}
% --------------------------- %
...
\answerfin{It says that "the formulas look similar to the synthetic estimators", because they are exactly same.  They merely have a modified design matrix.}
% --------------------------- %
